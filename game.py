...
def run_game():
    ...
    posns = pos_board(constants.start_x, constants.start_y, constants.inc_x - constants.x_gap, constants.inc_y - constants.y_gap, constants.inc_x, constants.inc_y, board)

    pygame.init()
    screen = pygame.display.set_mode((constants.total_width, constants.total_height))
    running = True
...
def draw_game(gamestate, screen):
    ...
    for i in range(len(wins)):
        rect = pygame.Rect(start_x, start_y, 4 * constants.inc_x - constants.y_gap, constants.height)
        pygame.draw.rect(screen, constants.colors[colors2[i]], rect, 0, 10)
        base_font = pygame.font.Font(None, constants.font_size)
        text = base_font.render(str(wins[i][0].total), True, constants.colors["eerie black"])
        screen.blit(text, (start_x + 50 * constants.scale_factor, start_y + 40 * constants.scale_factor))
